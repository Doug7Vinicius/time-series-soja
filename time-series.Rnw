...
<<script1, message=FALSE, warning=FALSE, echo=FALSE, fig.cap="Indicador de Preço da Soja no período de outubro de 1997 até outubro de 2022", fig.width=10,fig.height=6>>=

library(ggfortify)
library(magrittr) # for piping %>%
# Plot ts objects
autoplot(soja, xlab = "Tempo", ylab = "Valor (Reais por saca de 60 kg)")

@


<<script2, message=FALSE, warning=FALSE, echo=FALSE, fig.cap="", fig.height=4>>=
soja_decomp <- stl(soja, s.window = 21)
autoplot(soja_decomp)
@
...
soja <- read_xls("...dataset/soja.xls") #
